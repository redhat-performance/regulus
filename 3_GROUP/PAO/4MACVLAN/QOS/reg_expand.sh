...
envsubst '$TPL_MACVLAN'  < ${REG_TEMPLATES}/run-3types-14pods.sh.template > ${MANIFEST_DIR}/run-3types.sh

# generate node-config w/o custom resources. 
export TPL_RESOURCES=',resources:default:$pwd/resource-static-Ncpu.json'
envsubst '$MCP,$TPL_RESOURCES,$TPL_MACVLAN' < ${REG_TEMPLATES}/base-pao-node-config.template > ${MANIFEST_DIR}/node-config
...
# generate annotation. 
envsubst '' < ${REG_COMMON}/annotations-macvlan-pao-qos.json.template  > ${MANIFEST_DIR}/annotations.json
