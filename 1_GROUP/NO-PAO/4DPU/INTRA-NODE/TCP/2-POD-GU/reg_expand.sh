#!/bin/bash
# uperf DPU,NO-PAO,IPv4,INTRA_NODE,2 Pods, GU
...
if [ "${SUBSET_TESTS}" = "NIC-MODE" ]; then
    ...
    export TPL_MVPARAMS=${TPL_MVPARAMS:-r4-tcp-mv-params.json.template}
else
...
envsubst '\$TPL_QOS,$TPL_PAO,$TPL_DPF,\$TPL_NUMCPUs,$TPL_NUMCPUS,$TPL_QOS,$MCP,$TPL_PAO,$TPL_SCALE_UP_FACTOR,$TPL_TOPO' < ${REG_TEMPLATES}/run.sh.template > ${MANIFEST_DIR}/run.sh
