...
A web-based interactive dashboard for visualizing and analyzing performance benchmark reports generated by the build_report tool.
...
**Method 1: From build_report directory (Recommended)**
```bash
...
```bash
python3 /full/path/to/build_report/dashboard/run_dashboard.py --reports /tmp/reports
```
...
- Check that reports were generated by build_report tool with `--formats json`

### Dashboard shows "Loaded reports: 0"
- The reports directory is empty or has no JSON files
- Generate reports first:
  ```bash
  ./build_report --formats json --output /tmp/reports/test-report
  ```
- Use the sample report for testing:
  ```bash
  python3 dashboard/run_dashboard.py --reports .
  ```
