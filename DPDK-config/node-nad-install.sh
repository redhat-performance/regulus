...
function config_SriovNetworkNodePolicy {
    ...
    # 
    if [ ! -f ./templates/${REG_DPDK_NIC_MODEL}/sriov-node-policy.yaml.template ]; then
        echo "./templates/${REG_DPDK_NIC_MODEL}/sriov-node-policy.yaml.template not exist"
        echo "Please check for valid NIC_MODEL in setting.env"
        exit
    fi

    echo "Using templates/${REG_DPDK_NIC_MODEL}/sriov-node-policy.yaml.template "
 
    envsubst '$MCP,$REG_DPDK_NIC_1, $REG_DPDK_NIC_2,$SRIOV_MTU' < templates/${REG_DPDK_NIC_MODEL}/sriov-node-policy.yaml.template > ${MANIFEST_DIR}/sriov-node-policy.yaml
    echo "generating ${MANIFEST_DIR}/sriov-node-policy.yaml: done"
    # step 2 - apply

    if oc get SriovNetworkNodePolicy sriov-dpdk-net -n openshift-sriov-network-operator  2>/dev/null; then
        echo "SriovNetworkNodePolicy exists. Skip creation"
    else
        echo "create SriovNetworkNodePolicy ..."
        echo CMD: oc create -f ${MANIFEST_DIR}/sriov-node-policy.yaml
...
function create_network {
    envsubst < templates/network-attachment-dual-pf-definition.yaml.template > ${MANIFEST_DIR}/net-attach-def.yaml

    #return # NAD namespace must be pod's namespace. So let run.sh creates to correct NS

    if oc get network-attachment-definition/sriov-dpdk-net  &>/dev/null; then
