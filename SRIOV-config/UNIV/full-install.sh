...
function install_sriov_operator {
    ...
    if oc get Subscription sriov-network-operator-subscription -n openshift-sriov-network-operator &>/dev/null; then 
        echo $LINENO  "SRIOV Operator already installed: done"
    else
...
echo $LINENO "next is label node in ${WORKER_LIST} with ${MCP} label"
prompt_continue
...
echo $LINENO "next wait_mcp"
prompt_continue

# MCP goes to UPDATING after add label
wait_mcp

echo $LINENO "next is add realloc to machineconfig "
prompt_continue
...
# Probe whether the NIC can allocate VFs without pci=realloc.
...
if [[ "${probe_err,,}" == *"cannot allocate memory"* ]]; then
    ...
    echo "ERROR: VF probe failed with unexpected error: $probe_err"
    exit 1
else
    echo "NIC can allocate ${NUM_VFS} VFs, pci=realloc not needed"
fi
