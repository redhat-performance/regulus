...
function config_SriovNetworkNodePolicy {
    ...
    echo "Acquiring SRIOV interface PCI info from worker node ${WORKER_LIST} Hey ..."
...
function create_network {
    ...
    if oc get network-attachment-definition/$DIR-sriov-net -n openshift-sriov-network-operator  &>/dev/null; then
        echo "networkAttachment exists. Skip creation"
    else
        echo "create SriovNetwork ..."
        oc create -f ${MANIFEST_DIR}/net-attach-def.yaml
        echo "create networkAttachment net-attach-def.yaml  done"
    fi
}
