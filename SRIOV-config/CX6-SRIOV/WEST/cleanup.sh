...
if oc get SriovNetwork/$VENDOR-$DIR-testpmd-sriov-network -n openshift-sriov-network-operator &>/dev/null; then
    ...
    oc delete -f ${MANIFEST_DIR}/testpmd-sriov-network.yaml
...
if [ $SHORT == true ]; then
  ...
  echo "short. No removing mcp-$VENDOR-vf mcp as this is the WEST network"
  exit
fi


echo "Continue if you want to also remove the mcp-mlx-vf mcp  ..."
prompt_continue
