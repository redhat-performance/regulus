...
echo "next is creating mcp-intel-vf  mcp"
prompt_continue

# step 2 - Create mcp-intel-vf mcp

function configure_mcp {
    if oc get mcp mcp-intel-vf  &>/dev/null; then
        echo "mcp mcp-intel-vf exists. No need to create new"
    else
        echo "create mcp for mcp-intel-vf  ..."
        mkdir -p ${MANIFEST_DIR}
        envsubst < templates/mcp-intel-vf.yaml.template > ${MANIFEST_DIR}/mcp-intel-vf.yaml
        oc create -f ${MANIFEST_DIR}/mcp-intel-vf.yaml
        echo "create mcp for mcp-intel-vf: done"
    fi
}
