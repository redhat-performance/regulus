#!/bin/bash
# iperf IPv4,internode, N Pods. bitrate: half pods transmit each direction, step up bitrate
...
envsubst '$TPL_QOS,$MCP,$TPL_PAO,$TPL_SCALE_UP_FACTOR,$TPL_TOPO,$TPL_RESOURCES' < ${REG_TEMPLATES}/run.sh.template > ${MANIFEST_DIR}/run.sh
